...
namespace Z.Listening.Admin.WebAPI
{
    public class EncodingEpisodeHelper
    {
        ...
        {
            string redisKey = $"Listening.EncodingEpisodeIdsOfAlbum.{albumId}";
            return redisKey;
        }
        ...
        public async Task AddEncodingEpisodeAsync(Guid episodeId, EncodingEpisodeInfo episode)
        {
            ...
            string keyForEncodingEpisodeIdsOfAlbum = GetKeyForEncodingEpisodeIdsOfAlbum(episodeId);
            await db.SetAddAsync(keyForEncodingEpisodeIdsOfAlbum, episode.ToJsonString());//保存这个album下所有待转码的episodeId
        }
        ...
        public async Task UpdateEpisodeStatusAsync(Guid episodeId, string status)
        {
            ...
            await db.StringSetAsync(redisKeyForEpisode, episode.ToString());
        }
