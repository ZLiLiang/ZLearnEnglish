...
using Z.Commons;
using Z.EventBus;
using Z.FileService.SDK.NETCore;
using Z.JWT;
using Z.MediaEncoder.Domain;
using Z.MediaEncoder.Domain.Entities;
using Z.MediaEncoder.Infrastructure;
using Z.MediaEncoder.WebAPI.Options;

namespace Z.MediaEncoder.WebAPI.BgServices
{
    public class EncodingBgService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken = default)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                foreach (EncodingItem readItem in readyItems)
                {
                    try
                    {
                        await ProcessItemAsync(readItem, stoppingToken);//因为转码比较消耗cpu等资源，因此串行转码
                    }
                    catch (Exception ex)
                    {
                        readItem.Fail(ex);
        ...
        private async Task<(bool ok, FileInfo sourceFile)> DownloadSrcAsync(EncodingItem item, CancellationToken token)
        {
            //开始下载源文件
            string tempDir = Path.Combine(Path.GetTempPath(), "MediaEncodingDir");
            //源文件的临时保存路径
            string sourceFullPath = Path.Combine(tempDir, Guid.NewGuid() + "." + Path.GetExtension(item.Name));
            FileInfo sourceFile = new FileInfo(sourceFullPath);
            Guid id = item.Id;
            sourceFile.Directory!.Create();//创建可能不存在的文件夹
            logger.LogInterpolatedInformation($"Id={id}，准备从{item.SourceUrl}下载到{sourceFullPath}");
            HttpClient httpClient = httpClientFactory.CreateClient();
            var statusCode = await httpClient.DownloadFileAsync(item.SourceUrl, sourceFullPath, token);
            if (statusCode != HttpStatusCode.OK)
            {
                logger.LogInterpolatedWarning($"下载Id={id}，Url={item.SourceUrl}失败，{statusCode}");
                sourceFile.Delete();
        ...
        private Task<Uri> UploadFileAsync(FileInfo file, CancellationToken token)
        {
            Uri urlRoot = optionFileService.Value.UrlRoot;
            FileServiceClient fileService = new FileServiceClient(httpClientFactory, urlRoot, optionJWT.Value, tokenService);
            return fileService.UploadAsync(file, token);
        }
        ...
        private async Task<bool> EncodeAsync(FileInfo srcFile, FileInfo destFile, string outputFormat, CancellationToken token)
        {
            ...
            try
            {
                await encoder.EncodeAsync(srcFile, destFile, outputFormat, null, token);
            }
            catch (Exception ex)
            {
        ...
        /// <param name="token"></param>
        /// <returns></returns>
        private async Task ProcessItemAsync(EncodingItem encItem, CancellationToken token)
        {
            Guid id = encItem.Id;
            ...
            encItem.Start();
            await dbContext.SaveChangesAsync(token);//立即保存一下状态的修改
                                                    //发出一次集成事件
            (var downloadOk, var srcFile) = await DownloadSrcAsync(encItem, token);
            if (!downloadOk)
            {
                encItem.Fail($"下载失败");
                return;
            ...
            try
            {
                ...
                string outputFormat = encItem.OutputFormat;
                var encodingOK = await EncodeAsync(srcFile, destFile, outputFormat, token);
                if (!encodingOK)
                {
                    encItem.Fail($"转码失败");
                    return;
                ...
                Uri destUrl = await UploadFileAsync(destFile, token);
